...
def makePlot(g_pulls, h_pulls, avg=0):
    ...
    canvas.SaveAs("%s/decay_mode_independence_%s.png" % (outDir, args.type))
    canvas.SaveAs("%s/decay_mode_independence_%s.pdf" % (outDir, args.type))    
...
if __name__ == "__main__":
    ...
    outDir = "/eos/user/l/lia/FCCee/MidTerm/%s/plots_checks/" % flavor  
    ...
    if args.type == "mass":
        ...
        if flavor == "ee":
            cut_labels = [r"No selection", r"$\geq 1 e$ + ISO", r"$\geq 2 e$ + OS", r"$86 < m_{e^{+}e^{-}} < 96$", r"$20 < p_{e^{+}e^{-}} < 70$", r"$\cos(\theta_{miss}) < 0.98$", r"$120 < m_{rec} < 140$"]
    elif args.type == "xsec":
    ...
    f = open("%s/decay_mode_independence_%s.txt" % (outDir,args.type), 'w')
    sys.stdout = f
